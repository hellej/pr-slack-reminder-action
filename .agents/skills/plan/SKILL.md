...
- **Third-party client libraries** — if the plan needs a new method/capability from `github.com/google/go-github`, `github.com/slack-go/slack`, or any other dependency, verify it exists in that library's source or official docs first — check the local module cache, a vendor dir, or a local checkout (ask the user for its path, or search near the repo). Never assume or guess a method or parameter exists

## Mandatory Plan Steps

1. Create the plan file in `.local/plans/` (gitignored, untracked), unless the user says otherwise
2. Consider a pre-refactor step: for bigger features, restructuring existing code first — often renaming things so the new feature/concept lands as an explicit, self-evident diff — can make the real change smaller, safer, and more explicit. Propose it as a separate step before the main implementation when it earns its keep. Optional; skip for small changes
...
2. The target shape: the resulting architecture/feature, if not already fully covered by the requirements. Always call out changes to action inputs (`action.yml`) here
...
5. The full steps, each naming the files/packages it touches, in that same order — the order they're written IS the implementation order, never a separate order/sequence table
...
- Discarded options are mentioned in one line at most, never elaborated on
