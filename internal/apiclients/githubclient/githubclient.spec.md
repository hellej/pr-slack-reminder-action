...
- `FindOpenPRs` reads PRs over the GraphQL API in two phases: one request listing the open PRs of every repository, then reviews and comments for the capped set in batches of 25 PRs per request; `GetPRs` and `FetchLatestArtifactByName` stay on REST
- Review/comment data collected per PR is capped — `FindOpenPRs`: 100 reviews and 100 timeline comments, and review comments are not read at all (their authors always have a review of their own); `GetPRs`: 200 reviews, 100 PR comments and 100 timeline comments
- `FindOpenPRs`'s enrichment batches and `GetPRs`'s per-PR fetches are throttled to `DefaultGitHubAPIConcurrencyLimit` (3) concurrent GitHub API calls
...
- `FindOpenPRs` reads a PR's first 100 labels (GitHub's maximum page size) where REST returned them unpaginated — a PR with more labels can slip past `ignored-labels` or fail a `labels` allow-list
