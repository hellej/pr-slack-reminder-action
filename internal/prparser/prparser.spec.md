...
- `IsInactiveAsOf(asOf)` is the same threshold measured against a given time instead of the wall clock, for a caller that can't read the clock itself
- Unknown activity (a zero `UpdatedAt`) yields empty activity text, and counts as neither recently updated nor inactive
