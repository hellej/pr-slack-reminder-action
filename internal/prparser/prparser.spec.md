...
- `IsIdle` is true when `UpdatedAt` is over 48 hours old. The threshold is hardcoded
- Unknown activity (a zero `UpdatedAt`) is not stale: empty activity text and not idle
