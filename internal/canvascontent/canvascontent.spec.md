...
- Drafts whose update time is older than `MaxDraftPRInactivity` (60 days) are left out, via `prparser.PR.IsActiveAsOf(GeneratedAt, MaxDraftPRInactivity)`. A draft with a zero update time is kept: unknown is not stale
- At most `MaxInactiveWIPPRs` (5) drafts without recent activity reach the WIP section, the 5 most recently touched of them, via `prparser.PR.IsActiveAsOf(GeneratedAt, prparser.RecentActivityThreshold)`. Recently touched drafts are never capped, nor are drafts with a zero update time. The cap runs after the staleness prune, on the whole WIP list rather than per repository
