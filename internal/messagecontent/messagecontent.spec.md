...
Structures PR views and content settings into a `Content` value ready for `messagebuilder`.
...
- Grouped case: PRs are bucketed by repository via `prview.GroupPRsByRepositories` (alphabetical by repository path); this package adds each bucket's heading prefix, link label and link to the repository's GitHub pulls page (`models.Repository.GetPullsURL`)
- `Content.HasPRs()` reports whether there's anything to show, in either mode

## Doesn't Do

- Doesn't sort or filter PRs within a group — order is whatever it was given
