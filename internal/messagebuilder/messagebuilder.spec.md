...
- The message has no heading block. Its first block is `NoOpenPRsText` as a plain line when that is set, otherwise its first non-empty section
- Each non-empty section is one `rich_text` block holding a bold heading, then its rows. An empty section renders nothing, heading included
- The headings are this package's own display text: `✅ Ready to merge`, `💬 Waiting for author`, `👀 Waiting for review`, `🚀 Recently merged`
- Grouped-by-repository case: inside that same section block, an unbolded repository sub-heading carrying the repository link from [internal/messagecontent](../messagecontent/messagecontent.spec.md) and a bulleted list, per repository
- One spacing block sits between rendered sections
...
- The block cap is unreachable in practice: the layout spends at most 9 blocks, whatever it lists, because a whole section goes in one block. Per-block text length is what a huge section would run into instead, and nothing checks it
