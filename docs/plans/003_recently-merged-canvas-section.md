...
- **[Bump the Slack SDK](https://github.com/test-org/repo-two/pull/2)** _merged 2 hours ago_ by Bob Brown 🚀
...
- Row: linked title, merge text in italics, author, then 🚀. The text reads `merged 2 hours ago`, prefixed like WIP's `updated 5 hours ago`, so it cannot be misread as the age an open row shows in the same style. The marker is trailing, matching `messagebuilder.buildPRBulletPointBlock`.
...
**Private repositories.** The query reaches them: run live on 2026-08-22 against a private repository, it returned the merged PR with `repository.isPrivate` true, for a classic token holding `repo`. Undocumented is which permission counts as access, and the failure mode is silent, since no-access and no-matches are both `issueCount: 0`. Repeating that query with a token holding only `pull-requests: read`, as `README.md`'s `permissions:` block documents, stays untested: no such token was available. The README's permission block is unchanged, so a fine-grained token missing the permission would show an empty merged section and nothing else.
...
- Cost, measured live: **1 point per repository**, the same as the open listing, and 1 point in total if `labels` is dropped. The documented 30 requests/minute search limit is a REST figure and is no constraint at one request per canvas refresh.
- What that adds to a refresh: one request, and roughly double the listing cost. Three repositories with 40 open and draft PRs go from 5 points over 3 requests (3 listing + 2 enrichment batches) to 8 points over 4. Single digits against 5,000 points an hour.
...
- Add `func (pr PR) GetMergedText() string`: `""` when `GetMergedAt()` is nil, otherwise `"merged " + durationText(time.Since(*pr.GetMergedAt())) + " ago"`. It mirrors `GetActivityText`, which prefixes the same way. `durationText` is unexported, so this helper has to live in the package.
...
- `renderMergedPRRow`: `renderTitleLink`, then `GetMergedText()` in italics, then `renderAuthor`, then `" 🚀"`. No `renderReviewers` call. Empty merge text drops just that segment, as `renderWIPPRRow` does for unknown activity.
- The footer and `getCapText` are untouched.
- All 14 golden files in `internal/canvasbuilder/testdata/` gain the new section, re-recorded with `make update-test-snapshots`, plus two new ones pinning the section's own cases: `TestBuildMarkdownSnapshots-merged-PRs-could-not-be-fetched.md` for the failed-fetch line and `TestBuildMarkdownSnapshots-merged-PR-with-unknown-merge-time.md` for the dropped merge-text segment, which its WIP counterpart has a golden for too. `TestBuildMarkdownHasNoTopLevelHeading` gets a merged PR too.
...
- Mock transport, four changes:
  - `Post` dispatches on the query text, and a search query matches neither `"pullRequests("` nor `"pullRequest(number:"`, so it needs a `"search("` branch answering from new `MergedPRs` and `MergedPRsByRepo` options, the pair mirroring `PRs` and `PRsByRepo`, with a node JSON emitting `mergedAt` and an `issueCount`.
  ...
  - `ListPRsResponseStatus`, the only failure knob, is read inside `openPRsResponse`. The search branch gets its own, `MergedPRsSearchError`, which renders a 500, so the failed-merged-fetch test has a lever that leaves the open PR listing alone.
...
- `canvas_test.go`: merged fixtures come from a new `canvasTestMergedPRs()` next to `canvasTestPRs()`, since the two feed different mock options, and the section assertions gain `## Merged` and `_No merged PRs_`. New cases: a merged PR renders with 🚀 in merge order, a merge older than the window is left out, a repository filter excludes one repository's merged PR and not the other's, and a failed merged fetch still writes the canvas with the unavailable line while the run reports the error.
